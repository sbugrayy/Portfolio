...
const DEFAULT_EFFECT_OPTIONS = {
  ...
  movingAwaySpeed: [4, 10],
  movingCloserSpeed: [-10, -20],
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    if (appRef.current) {
      if (sphereState === 'thinking') {
        // Tam gaz ileri (yeni speedUp: 4 ile uTime 5 kat hızlı akacak, ilk haldeki hıza denk gelecek)
        ...
        // Konuşurken orta hız
        appRef.current.fovTarget = 80;
        appRef.current.speedUpTarget = 1;
      } else {
        // Beklerken çok yavaş, süzülerek
  ...
  return <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: -2 }} ref={hyperspeed}></div>;
};
